## 👋 Hello!

I am Artem Marchenko and I make it my business to defend the citizens of Saint Petersburg in cybersecurity. I like to solve difficult tasks and create something new. I actively participate in various contests, hackathons and projects, constantly expanding my knowledge and skills. Currently, I am studying at T-Generation on the course of information security in parallel β. 

*What is this profile about:*

Here you will find my personal projects, solutions to problems from contests, as well as code examples demonstrating my skills. I try to constantly improve my code and document my achievements. So far, there are not so many projects, but more to come!

*My skills:*

* *Programming languages:* Python, JavaScript, C++, Java, Go
* *Technologies:* HTML, CSS, React, Node.js, SQL, Git
* *Tools:* VS Code, Git, Docker
* *Other skills:* teamwork, the ability to learn quickly, problem solving


*My Projects:*

I am currently working on several interesting projects that will appear here soon. Stay tuned!


*Graduated from the Yandex Lyceum courses:* 
* Python Programming Fundamentals
* Python Industrial Programming
* Go Programming
* Go Web Development


*Graduated from Samsung IT School:*
* Mobile Game Development
* Mobile Application Development


Studied Telegram bot development at Innopolis University

Completed a course on Fullstack development in JavaScript from Yandex Practicum


*Participation in contests and hackathons:*

* DANO Data Analysis Hackathon (3 times)
* Mobile GameDev Bootcamp: Developed the Space Cleaner game. I got the maximum score for the project. Passed the LSPI
* NTO Final in Augmented Reality Technologies (2025)
* NTO Final in Information Security (2025)
* PROD Industrial Development Hackathon
* NTO Final in Information Security (2026)
* NTO Final in Mobile App Development (2026)
* Innopolis Open Information Security Final (2026)
* InnoCTF Final (2025)
* All-Russian Case Championship "KEY" by HSE (2024)
* PROD Final (2026)
* Prizewinner of the regional stage of VSOSH in Information Security (2026)


*P.S. * I am open to cooperation and new projects!
